...
int common_fn(int c,int mode)
{
 ...
 while (1)
 {
  ...
  switch(c){
   ...
   case CTRL_L :
   case 'l':if (mode==TEXT && c=='l')	
			return(0);
                  if (r->x == r->no_of_chars && mode==COMMAND)
                    beep();
		  else
		  if (r->x == (r->no_of_chars+1)) //&& mode==TEXT
		    beep();
		  else
		  {
		   if (mode==COMMAND)
   ...
   case KEY_BACKSPACE :	// backspace
		  ...
		  if (r->x > r->no_of_chars) // && mode==TEXT
		  {
		   r->x--;
		   wmove(textwin,buffer->y-1,screen_pos(r)-1);
		  } 
